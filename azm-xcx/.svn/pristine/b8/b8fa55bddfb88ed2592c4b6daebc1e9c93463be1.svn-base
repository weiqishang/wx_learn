...
Page({
  ...
  renmaiTips: function (rmNumber, viptype) {
      ...
      success: function (res) {
        ...
        if (look_Num == rmNumber) {
          if (viptype == 0) {
            ...
            wx.showModal({
              title: '使用提醒',
              content: '非VIP会员，每天只能查看' + rmNumber + '条',
              confirmText: "升级会员",
              cancelText: '体验会员',
              success: function (res) {
                console.log(res);
    ...
    /**
        ...
        if (lookNum == "") {
          wx.setStorageSync(datestr, 1);
          this.setData({ lookNum: 1 });
        } 
        else 
        {
          if ((lookNum + 1) > rmNumber) {
            wx.setStorageSync(datestr, rmNumber);
            this.setData({ lookNum: rmNumber });
          } else {
            wx.setStorageSync(datestr, lookNum + 1);
            this.setData({ lookNum: lookNum + 1 });
          }
        } */
  },
    ...
    wx.request({//请求拿到用户数据的接口
      ...
      success: function (res) {
        ...
        if (options.usertype != undefined) {
          that.setData({ usertype: options.usertype });
          var usertype = options.usertype;
          if (usertype == '1') {
            var tabs = ["100条直销人脉", "人脉订阅"];
            that.setData({ tabs: tabs });
          } else if (usertype == '2') {
            var tabs = ["100条微商人脉", "人脉订阅"];
            that.setData({ tabs: tabs });
          } else if (usertype == '3') {
            var tabs = ["100条宝妈人脉", "人脉订阅"];
            that.setData({ tabs: tabs });
          } else if (usertype == '4') {
            var tabs = ["100条大学生人脉", "人脉订阅"];
            that.setData({ tabs: tabs });
          } else if (usertype == '5') {
            var tabs = ["100条老板人脉", "人脉订阅"];
            that.setData({ tabs: tabs });
          } else if (usertype == '6') {
            var tabs = ["100条保险人脉", "人脉订阅"];
            that.setData({ tabs: tabs });
          } else if (usertype == '7') {
            var tabs = ["100条专享人脉"];
            that.setData({ tabs: tabs });

...
Page({
  ...
  shareCard7_24:function(){
    var personData = this.data.personData;
    wx.navigateTo({
      url: './mycard/mycard?personData=' + JSON.stringify(personData)+"&",
    })
  },
  ...
  checkIsZan: function () {//查询用户是否点赞过
    ...
    wx.request({
      ...
      success: function (res) {
        // success
        console.log(that.data.ownerId+"查询点赞返回" + xcxSession);
        console.log(res);
  ...
  drawImg2: function () {//保存图片的
    ...
    ctx.fillText(this.data.personData.belongCom, canvasW * (266 / 288), canvasH * (101 / 436));
    ...
    ctx.setTextAlign('right');
    ctx.fillText(teamNameTmp + "", canvasW * (266 / 288), canvasH * (132 / 436));
    ctx.setTextAlign('left');
    ctx.fillText('电话', canvasW * (22 / 288), canvasH * (166 / 436));
    ctx.setFillStyle('#70a0ff')
    ctx.setTextAlign('right');
    ctx.fillText(this.data.personData.phone, canvasW * (266 / 288), canvasH * (166 / 436));
    ctx.setFillStyle('black')
    ctx.setTextAlign('left');
    ctx.fillText('QQ号', canvasW * (22 / 288), canvasH * (199 / 436));
    ctx.setTextAlign('right');
    ctx.fillText(this.data.personData.qq, canvasW * (266 / 288), canvasH * (199 / 436));
    ctx.setTextAlign('left');
    ctx.fillText('微信', canvasW * (22 / 288), canvasH * (232 / 436));
    ctx.setTextAlign('right');
    ctx.fillText(this.data.personData.wechar, canvasW * (266 / 288), canvasH * (232 / 436));
  ...
  drawCardImg: function () {
    ...
    if (this.data.picurlXcxQr == '' || this.data.picurlXcxQr == null) {
      console.log("制造二维码传的owerId参数" + that.data.ownerId);
      var name = this.data.personData.name;
      //if (name.indexOf("测试") > -1) {
      if (this.data.condition == "test") {
        console.log("新的B接口");
        wx.request({
      ...
      //that.saveImgPath(url);//保存小程序码
      that.downLoadImg(this.data.picurlXcxQr);
    }
  ...
  shareCard: function () {
    ...
    //     xcxSession = wx.getStorageSync('xcxSession');
    //   }
    //   console.log("生成图片用的sessioin---:" + xcxSession);
    //   wx.request({
  ...
  onShow: function () {
    ...
    if (this.data.scene != "00") {
      return;
    }
    console.log("首页onShow2----------");
    var ownerId = wx.getStorageSync('myId');
    var xcxSession = wx.getStorageSync('xcxSession');
  ...
  onLoad: function (options) {
    ...
    var parentOne = options.parentOne
    if (parentOne != undefined) {//带参进来的
      wx.setStorageSync('parentOne', parentOne);
    }

    var scene = options.scene;// 535;//
    ...
    var ownerId = options.ownerId;//545;//
  ...
  sendServerToSetopenGId: function (shareTickets) {
    var xcxSession = wx.getStorageSync("xcxSession");
    wx.login({
      success: function (res) {//用户登录成功
        ...
        if (res.code) {
          ...
          wx.getShareInfo({
            ...
            success: function (res) {
              ...
              var encryptedData = res.encryptedData;
              wx.request({//发到服务器，进行解密数据
                url: 'https://m.ccyishe.com/page/xcx/pronumber!shareQt.jsp',
                method: "POST",
                data: {
                  encryptedData: encryptedData,
                  iv: iv,
  ...
  checkIsPersonOrTeam: function (ownerId) {
    wx.request({
      ...
      success: function (res) {
        ...
        wx.navigateTo({
          url: '../discover/tools/article/teamarticle/mteamarticle?ownerId=' + ownerId + '&isTeam=' + isTeam,
        })
      },
      fail: function (res) {
        // fail
  ...
  moveToTeamArticle:function(){
    var that = this;
    this.checkIsPersonOrTeam(that.data.ownerId);
  ...
  onShareAppMessage: function () {
    ...
    return {
      ...
      success: function (res) {
        ...
        if (myId != "") {
          if (res.shareTickets != undefined && res.shareTickets != "undefined" && res.shareTickets != "") {
            that.sendServerToSetopenGId(res.shareTickets);
          }
        }
